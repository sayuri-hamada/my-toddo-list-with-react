...
export const TodoCard: FC<Props> = memo((props) => {
  const { taskName, completed, index, onClickComplete ,onClickDelete, onClickBack} = props
    ...
    <div className={styles['todo-card']}>
      ...
      {completed ? (
        <div className={styles['todo-card__btn-wrp']}>
          <Btn className={styles['todo-card__btn-back']} onClick={()=> onClickBack(index)}>戻す</Btn>
        </div>
      ): (
        <div className={styles['todo-card__btn-wrp']}>
